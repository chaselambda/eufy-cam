...
const CROP_START_RATIO = 1500 / 2300; // Look at bottom camera and also ignore part of sidewalk
...
/**
 * Sharp pipeline cropped to the bottom-camera doorstep region — the region
 * the model classifies. Both the model input (cropAndScale) and the
 * pre-filter comparison build on this so their geometry cannot diverge.
 * @param {string} inputPath - Path to original image
 * @returns {Promise<import("sharp").Sharp>}
 */
export async function bottomCropPipeline(inputPath) {
...
export async function cropAndScale(inputPath) {
  ...
  const pipeline = await bottomCropPipeline(inputPath);
  await pipeline.resize({ width: TARGET_WIDTH }).toFile(tempPath);
  return tempPath;
}

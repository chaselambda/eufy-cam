...
export async function detectPackage(imagePath) {
  ...
  const prompt = `
  Determine if there's a package on the doorstep in the provided image. The package needs to be obvious. I.e. if there's some dark object that might be a package or might be a shadow and it's not clear, the answer should be no. The camera will be facing down, so you may see the legs of a person. This does not indicate a package.

  First, respond with a description of the image, without consideration for packages. Just simply describe what you see in the image. Then, respond with true/false for if there is a package. Respond with ONLY valid JSON (no markdown, no explanation). Here's an example response:
